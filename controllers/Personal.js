...
const obtener_Personal = async (req, res) => {
    ...
    try {
        const [Personals] = await con.query(`
            select pl.id_Plaza, pe.status, pe.id_Personal, pe.nombre, pl.puesto, u.nombre as unidad, u.zona ,pl.nombre as plaza, pe.codigo, pl.nivel, pe.sindicalizado, pl.tabulador, u.zona,
            (
                SELECT COUNT(*) 
                FROM Historial h
